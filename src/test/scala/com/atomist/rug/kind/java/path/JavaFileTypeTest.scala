...
class JavaFileTypeTest extends AbstractTypeUnderFileTest {
  ...
  it should "map methodDeclaration to Func" in {
    val expr1 = "/File()/JavaFile()//Func()"
  ...
  it should "map formalParameterList to Args" in {
    val expr1 = "/File()/JavaFile()//Func()[/methodHeader/methodDeclarator/Identifier[@value='twoArgs']]//Args()"
    val rtn1 = expressionEngine.evaluate(manyMethodsProject, PathExpressionParser.parseString(expr1), DefaultTypeRegistry)
    assert(rtn1.right.get.size === 2)
  ...
  it should "map classDeclaration and methodDeclaration to Class and Func" in {
    val expr1 = "/File()/JavaFile()//Class()//Func()"
...
object JavaFileTypeTest {
  ...
  val ManyMethodsInClass = StringFileArtifact("ManyMethodsInClass.java",
    """
      ...
      | public void variableArgs(String ... names) {}
      |}
    """.stripMargin)
